    ...
    <div className="max-w-lg mx-auto p-4 sm:p-6 md:p-8 bg-white shadow-lg rounded-lg">
      ...
      <div className="space-y-6">
        <div className="p-4 bg-gray-50 rounded-lg">
          ...
          {["front", "back"].map((side) => (
            <div key={side} className="border-2 border-dashed border-gray-200 rounded-lg p-4 sm:p-6 text-center">
              <div className="flex justify-center mb-4">
                <Upload className="w-6 h-6 sm:w-8 sm:h-8 text-gray-400" />
              </div>
              <p className="font-medium mb-1 text-sm sm:text-base">{side === "front" ? "Front side" : "Back side"}</p>
              <p className="text-xs sm:text-sm text-gray-500">Choose, drag & drop, or take a photo</p>
          ...
          {isCameraOpen && (
            <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center">
              <div className="bg-white p-6 rounded-lg">
                ...
                <button onClick={captureImage} className="mt-4 bg-green-600 text-white py-2 px-4 rounded-lg">
                  Capture
                </button>
              </div>
            </div>
